...
app.get('/health', (_, res) => res.json({ ok: true, rooms: rooms.size, version: '0.7.4-combat-math-hand-redesign' }));
...
function serializeRoom(room, viewerId) {
  ...
  return {
    version: '0.7.4-combat-math-hand-redesign',
    code: room.code,
    status: room.status,
    phase: room.phase,
...
function startDeathLooting(room, victim, sourceCard, context = {}) {
  ...
  announce(room, 'effect', 'Death — Loot the Body', detail, sourceCard, { importance: 'major' });
...
function finishBodyLooting(room) {
  ...
  announce(room, 'effect', 'Body Looting Complete', count ? `${count} unclaimed card${count === 1 ? '' : 's'} were discarded.` : 'No cards remain to loot.', null, { importance: 'major' });
...
io.on('connection', (socket) => {
  socket.emit('ready', { version: '0.7.4-combat-math-hand-redesign' });
...
server.listen(PORT, () => {
  console.log(`Loot Goblins v0.7.4 Combat Visualization + Compact Hand Clarity listening on ${PORT}`);
});
