...
app.get('/health', (_, res) => res.json({ ok: true, rooms: rooms.size, version: '0.7.1-card-frame-inspector' }));
...
io.on('connection', (socket) => {
  socket.emit('ready', { version: '0.7.1-card-frame-inspector' });
...
server.listen(PORT, () => {
  console.log(`Loot Goblins v0.7.1 Mechanical QA / Rules Lock listening on ${PORT}`);
});
